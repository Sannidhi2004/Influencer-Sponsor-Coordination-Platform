...
#Set up influencer profile and hence, add user to the influencer table
...
#For the influencer, to update their (own) account
@app.route('/update_influencer', methods=["GET", "POST"])
@login_required
...
#For the influencer, to delete their (own) account
...
#For the influencer, Influencer can see all the PUBLIC CAMPAIGNS here
...
#For the influencer,to search for public campaigns based on name, niche, maximum and minimum budget
...
def search_campaigns():
    ...
    campaigns = query.all()
    influencers = Influencer.query.all()  # Fetch influencers
    current_user_id = current_user.id  
    return render_template('influencer_campaign.html', campaigns=campaigns, influencers=influencers, current_user_id=current_user_id)


#For the influencer, to view the ADS OF THE PRIVATE CAMPAIGNSs sent to them
...
def influencer_priv_adrequests(influencer_id):
    ...
    for ad in ad_requests:
        ...
        if campaign and campaign.visibility == 'Private':
...
#For the influencer, VIEW ADS of the PUBLIC CAMAIGN
...
#For the influencer,to 'REQUEST THE SPONSOR' for any particular ad of the public campaign
...
#For the influencer, to 'NEGOTIATE' for any particular ad of the public campaign
...
#FOR AD REQUESTS OF THE PRIVATE CAMPAIGNS (sent to a particular influencer)

#To ACCEPT THE AD REQUEST (of private campaigns)
...
#To REJECT THE AD REQUEST (of private campaigns)
...
#To NEGOTIATE THE AD REQUEST (of private campaigns)
...
#For the influencer, to see the ads that have the 'Accepted' status and but are yet to be completed
...
#For the influencer, to mark an ad request as 'COMPLETED'.
...
#For the influencer, to view the statistics of the received and completed ad requests.
...
def sponsor_dashboard():
    ...
    '''
    flagged_campaigns = {flag.item_id for flag in Flag.query.filter_by(item_type='Campaign').all()}
    public_campaigns = Campaign.query.filter_by(user_id=sponsor_id, visibility='Public').filter(Campaign.id.notin_(flagged_campaigns)).all()
    private_campaigns = Campaign.query.filter_by(user_id=sponsor_id, visibility='Private').filter(Campaign.id.notin_(flagged_campaigns)).all()
...
#For the sponsor, to delete their (own) account
...
#For the sponsor, to VIEW THE INFLUENCERS 
...
#For the sponsor, to SEARCH FOR INFLUENCERS WHILE VIEWING THEM
...
def search_influencers():
    ...
    return render_template('view_influencers_to_sponsor.html', influencers=filtered_influencers)




#To CREATE A CAMPAIGN (public/private)
...
#To SHOW CAMPAIGNS (public/ private)(will not show flagged campaigns)
...
#For the sponsor, for creating ads of the private campaigns (for a specific influencer)
...
#For the sponsor, to SEARCH FOR INFLUENCERS while making an ad request for PRIVATE CAMPAIGN 
...
#For the sponsor, to create ads for a public campaign
...
#For the sponsor, to VIEW ADS for a public/private campaign
...
#For the sponsor, to UPDATE ADS for a public/private campaign
...
#For the sponsor, to DELETE ADS for a public/private campaign
...
#For the sponsor, TO MANAGE AD REQUESTS and NEGOTIATIONS (by the influencer) for the ads of public campaigns
@app.route('/manage_negotiated_ads', methods=['GET'])
@login_required
def manage_negotiated_ads():
    flagged_campaign_ids = {flag.item_id for flag in Flag.query.filter_by(item_type='Campaign').all()}
    ...
    visible_negotiated_ads = [ad for ad in negotiated_ads if ad.campaign_id not in flagged_campaign_ids]
    return render_template('manage_negotiated_ads.html', ads=visible_negotiated_ads)
...
#For the sponsor, ACCEPT THE REQUEST (sent by the influencer)
...
#For the sponsor, REJECT THE REQUEST (sent by the influencer)
...
#For the sponsor, UPDATE AND ACCEPT THE REQUEST (if there's a negotiation from the influencer's side)
...
#For the sponsor, VIEW THE COMPLETED ADS
...
#For the sponsor, to COMPLETE TRANSACTIONS AND PAY the influnencer for the completed ads
...
#For the admin, to view all the flagged/ not flagged influencers, sponsors campaigns(public/private/completed/ongoing) & their ad requests
...
def admin_profile():
    ...
    sponsor_dict = {sponsor.id: sponsor.username for sponsor in sponsors}
    ...
    return render_template('admin_profile.html',flagged_items = flagged_items, ongoing_campaigns=ongoing_campaigns, completed_campaigns=completed_campaigns, influencers=influencers, campaigns=campaigns, sponsors=sponsors,influencer_dict=influencer_dict,
        sponsor_dict=sponsor_dict, campaign_dict=campaign_dict)

#For the admin, to see all the influencers, sponsors and their campaigns to FLAG them 
...
#For the admin, to VIEW INFLUENCERS AND THEIR ACTIVE AND COMPLETED ADS on the profile page
...
#For the admin, to VIEW SPONSORS AND THEIR PRIVATE/PUBLIC CAMPAIGNS on the profile page
...
#For the admin, to VIEW CAMPAIGNS & ADS on the profile page
...
#For the admin, to FLAG InFLUENCERS, SPONSORS AND CAMPAIGNS 
...
#For the admin, to REMOVE THE FLAG InFLUENCERS, SPONSORS AND CAMPAIGNS 
